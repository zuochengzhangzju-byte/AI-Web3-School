...
-   **GitHub 仓库：**[ai-web3-school-cohort-0](https://github.com/gnihTehT/ai-web3-school-cohort-0)
    
-   **产物文件：**[notes/week-01/ai\_learn.py](https://github.com/gnihTehT/ai-web3-school-cohort-0/blob/main/notes/week-01/ai_learn.py)
    
-   **运行方式：** 终端执行 `python3 ai_learn.py [模式]`，Python 3 零依赖
    

**解决什么学习问题**Week 1 学习了 10+ 个 AI 概念和 4 个 Web3 概念，但**被动阅读笔记 → 容易看完就忘**。 这个工具把复习变成三种交互模式：

-   **测验** — 看到描述 → 主动回忆概念名（主动回忆是最高效的记忆方式）
    
-   **查阅** — 快速翻查某个概念的解释/例子/误区
    
-   **对比** — 让容易混淆的概念对（Agent vs Workflow、EOA vs 智能账户）并排对比，明确边界
    
-   **漫游** — 随机的"不期而遇"，适合碎片时间刷概念 —
    

**用户如何交互方式 A：CLI 参数（一行命令）**

```
python3 ai_learn.py quiz      # 进入测验模式
python3 ai_learn.py explain   # 查阅概念
python3 ai_learn.py compare   # 概念对比
python3 ai_learn.py explore   # 随机漫游
python3 ai_learn.py list      # 列出所有概念
```
...
```
  ...
  🎯 例子:
     订机票 Agent 接到'帮我订下周五去上海的机票'后，自动搜索航班...

  ⚠️  常见误区:
     很多人把'会聊天的 AI'和 Agent 混为一谈...
```
...
```
  ...
  ─────────────────────────┼─────────────────────────┤
  能自主规划、调用工具、执行多步骤...│ 把多个 AI 调用按逻辑顺序串联...│
  ─────────────────────────┴─────────────────────────┘
